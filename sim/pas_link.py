...
class ProjectAirSimLink:
 ...
 def connect(self, timeout_s=15.0):
  ...
  while time.monotonic() - t0 < timeout_s:
   try:
    self._client = pas.ProjectAirSimClient(
 ...
 def _wait_for_message(self, mt, to):
  return True
 def _wait_for_custom_mode(self, m, t):
  return True
 ...
 def request_arm(self, to=2.0):
  self.telemetry.armed = True
  return True
 def request_disarm(self, to=2.0):
  self.telemetry.armed = False
  return True
 def start_offboard(self, to=2.0):
  return True
 ...
 def request_land(self, to=4.0):
  try:
   ...
   pass
  self.state = LinkState.PX4_LAND
  return True

 def request_rtl(self, to=4.0):
  self.state = LinkState.PX4_RTL
  return True
 ...
 def stop_setpoint_stream(self):
  self._streaming = False
  self.telemetry.setpoint_streaming = False
 ...
 def wait_for_required_telemetry(self, to=2.0):
  ...
  while time.monotonic() - t0 < to:
   self._refresh_telemetry()
   if self.telemetry.position_valid:
    return True
   time.sleep(0.1)
  return False

 def setpoint_age_ms(self):
  if self._last_setpoint_ts == 0:
   return 9999.0
